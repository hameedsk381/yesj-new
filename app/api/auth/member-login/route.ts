...
export async function POST(req: NextRequest) {
  const limit = checkRateLimit(`member-login:${getClientId(req)}`, {
    interval: 60_000,
    maxRequests: 10,
  });
  if (!limit.success) {
  ...
  try {
    ...
    const member = await db.query.registrations.findFirst({
      where: eq(registrations.emailId, email.toLowerCase()),
    });

    if (!member || !member.hashedPassword) {
      return NextResponse.json({ error: "No membership found for this email" }, { status: 400 });
    }

    if (!(await verifyPassword(password, member.hashedPassword))) {
      return NextResponse.json({ error: "Incorrect email or password" }, { status: 400 });
    }

    const token = await createAccessToken({ sub: member.id.toString(), role: "member", email: member.emailId });
